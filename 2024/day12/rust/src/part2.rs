use crate::common::{get_sides_count, parse_input, Input, Point};
...
fn solve_inner(input: Input) -> u64 {
    ...
    for (k, v) in regions.iter() {
        ...
        let sides: u64 = get_sides_count(k, v, &input);
        println!("{} Area {}, sides {}", k, area, sides);
...
mod test {
    ...
    fn test() {
        ...
        assert_eq!(solve_inner(input), 1206);
    }
    ...
    fn test_small() {
        let raw = r###"AAAAAA
                        ...
                        AAAAAA"###;
        let input = parse_str(raw);
        assert_eq!(solve_inner(input), 368);
    }
    ...
    fn test_smaller() {
        let raw = r###"EEEEE
                       ...
                       EEEEE"###;
        let input = parse_str(raw);
        assert_eq!(solve_inner(input), 236);
    }
    ...
    fn test_small1() {
        let raw = r###"OOOOO
                       ...
                       OOOOO"###;
        let input = parse_str(raw);
        assert_eq!(solve_inner(input), 436);
    }
}
